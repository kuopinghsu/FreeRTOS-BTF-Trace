"""Uniform demo voice packs: install / export / normalize (package-mirror layout)."""
...
TEXT = DEMO_DIR / "attachments" / "text"
VOICE = DEMO_DIR / "demo" / "voice"
...
class DemoVoicePackTests(unittest.TestCase):
    def test_shipped_demo_uses_lang_folders(self) -> None:
        self.assertTrue((TEXT / "en" / "01_title.txt").is_file())
        self.assertTrue((TEXT / "en" / "voice.json").is_file())
        self.assertTrue((VOICE / "en" / "01_title.mp3").is_file())
        self.assertTrue((VOICE / "en" / "voice.json").is_file())
        ...
        data = json.loads((VOICE / "en" / "voice.json").read_text(encoding="utf-8"))
    ...
    def test_runner_resolves_voice_en_without_flat_files(self) -> None:
        clip = VOICE / "en" / "01_title.mp3"
        xml_dir = DEMO_DIR / "demo"
        got = dr.resolve_media_path(
            str(xml_dir / "voice" / "01_title.mp3"),
            {
                "XML_DIR": str(xml_dir),
                "CWD": str(DEMO_DIR),
                "BTF": str(DEMO_DIR),
    ...
    def test_normalize_moves_flat_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            demo = Path(tmp)
            (demo / "attachments" / "text").mkdir(parents=True)
            (demo / "demo" / "voice").mkdir(parents=True)
            (demo / "attachments" / "text" / "01_title.txt").write_text("hello", encoding="utf-8")
            (demo / "demo" / "voice" / "01_title.mp3").write_bytes(b"en")
            stats = dv.normalize_demo(demo, "en")
            self.assertEqual(stats["moved"], 2)
            self.assertTrue((demo / "attachments" / "text" / "en" / "01_title.txt").is_file())
            self.assertTrue((demo / "demo" / "voice" / "en" / "01_title.mp3").is_file())
            self.assertFalse((demo / "attachments" / "text" / "01_title.txt").exists())
            self.assertFalse((demo / "demo" / "voice" / "01_title.mp3").exists())
    ...
    def test_export_install_roundtrip(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (src / "attachments" / "text" / "zh-tw").mkdir(parents=True)
            (src / "demo" / "voice" / "zh-tw").mkdir(parents=True)
            (src / "attachments" / "text" / "zh-tw" / "01_title.txt").write_text("你好", encoding="utf-8")
            (src / "demo" / "voice" / "zh-tw" / "01_title.mp3").write_bytes(b"zh")
            dv.write_manifest(src / "demo" / "voice" / "zh-tw" / "voice.json", "zh-tw", "中文")
            ...
            (dst / "attachments" / "text" / "en").mkdir(parents=True)
            ...
            self.assertEqual(
                (dst / "attachments" / "text" / "zh-tw" / "01_title.txt").read_text(encoding="utf-8"),
                "你好",
            )
            self.assertEqual((dst / "demo" / "voice" / "zh-tw" / "01_title.mp3").read_bytes(), b"zh")
            man = json.loads((dst / "demo" / "voice" / "zh-tw" / "voice.json").read_text(encoding="utf-8"))
    ...
    def test_install_loose_files_with_lang_flag(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((demo / "attachments" / "text" / "de" / "01_title.txt").is_file())
            self.assertTrue((demo / "demo" / "voice" / "de" / "01_title.mp3").is_file())

    def test_sync_xml_rewrites_languages(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            demo = Path(tmp)
            (demo / "attachments" / "text" / "en").mkdir(parents=True)
            (demo / "demo" / "voice" / "ja").mkdir(parents=True)
            (demo / "attachments" / "text" / "en" / "01.txt").write_text("hi", encoding="utf-8")
            (demo / "demo" / "voice" / "ja" / "01.mp3").write_bytes(b"ja")
            xml = demo / "demo" / "script.xml"
            xml.write_text(
                '<?xml version="1.0"?>\n<demo>\n  <meta>\n    <title>x</title>\n  </meta>\n</demo>\n',
                encoding="utf-8",
            )
