...
**使用方式：**將優先權提升與 **Mutex / Semaphore**、**等待工作 × 持有者**及時間軸配對。優先權變更也可能由應用程式主動控制，不一定是優先權繼承。提升通常表示系統已進行緩解，仍需查看等待時間是否影響需求。
...
**使用方式：**事件配對有問題時，應先處理配對品質，再使用持有時間或阻塞推估。長時間持有但沒有其他等待工作，不一定有害；應搭配**Mutex 阻塞**、**等待工作 × 持有者**、優先權與時間軸。
...
**Shape Δ：**Execution Time、Blocking Time 與 Inter-Arrival Time 表格新增 **Shape Δ** 欄：該工作在 Baseline A 與 Candidate B 兩組樣本分布之間的雙樣本 Kolmogorov–Smirnov 統計量（`0` = 形狀相同，`1` = 完全不重疊）。此值與尺度無關，因此即使 Avg、Max 與 Δ 欄幾乎沒變，也能抓出尾端變寬或分布出現雙峰的情形。顯示「—」代表其中一側樣本太少（`< 3`）。
