...
describe('Auto investigate template + tool registration (web/desktop parity)', () => {
  it('registers auto_investigate as an agent template', () => {
    ...
    assert.match(tpl.prompt, /verify_claim/)
    assert.match(tpl.prompt, /challenge_conclusion/)
    assert.match(tpl.prompt, /Stop after verification/)
    assert.match(tpl.prompt, /Do not run what_if/)
