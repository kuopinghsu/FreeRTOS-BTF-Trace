...
describe('aiCase investigation lifecycle', () => {
  ...
  it('matches catalog knowledge and round-trips user templates', () => {
    ...
    assert.doesNotMatch(prompt, /Call these tools in order/)
    const report = investigationModePrompt('report')
    assert.match(report, /generate_report/)
    assert.match(report, /export_report/)
    assert.match(report, /Call generate_report, then/)
    assert.doesNotMatch(report, /only when the user asks/)
    ...
    assert.match(VALIDATE_EXPERIMENT_PROMPT, /Use validate_experiment/)
    assert.match(VALIDATE_EXPERIMENT_PROMPT, /VALIDATED/)
    assert.match(VALIDATE_EXPERIMENT_PROMPT, /candidate/)
  ...
  it('compacts findings, tools, and history in Compact mode', () => {
    ...
    // Start investigation uses template intent, not idle/triage guide stage.
    const autoTools = toolNamesForContextMode('balanced', 'auto_investigate')
    assert.ok(autoTools.includes('investigate'))
    assert.ok(autoTools.includes('correlate_events'))
    assert.equal(toolNamesForContextMode('balanced', 'idle').includes('investigate'), false)
    const fullTools = toolNamesForContextMode('full', 'triage')
    assert.ok(Array.isArray(fullTools))
    assert.ok(fullTools.includes('detect_anomalies'))
    assert.ok(fullTools.includes('verify_claim'))
    assert.equal(fullTools.includes('what_if'), false)
    assert.ok(fullTools.length < 40)
    ...
    assert.match(compact, /4 more finding/)
