...
describe('AI endpoint helpers', () => {
  it('aiListModels always arms a timeout even with an external signal', () => {
    const src = readFileSync(
      new URL('../src/utils/aiClient.js', import.meta.url),
      'utf8',
    )
  ...
  it('normalizeApiKey strips Bearer and quotes', async () => {
    const { normalizeApiKey, aiRequestHeaders } = await import('../src/utils/aiClient.js')
  ...
  it('aiChatCompletion times out like desktop (120s default)', async () => {
    const { aiChatCompletion, AI_CHAT_TIMEOUT_MS } = await import('../src/utils/aiClient.js')
  ...
  it('normalizeAiContext accepts snake_case and camelCase', async () => {
    const { normalizeAiContext } = await import('../src/utils/aiClient.js')
  ...
  it('buildAiUserMessage includes cursor region window', async () => {
    const {
      ...
      buildAiUserMessage,
      cursorRegionBounds,
    } = await import('../src/utils/aiClient.js')
