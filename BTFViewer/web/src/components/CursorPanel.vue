<template>
  <div class="cursor-panel" v-if="validCursors.length > 0">
    <div class="cursor-row" v-for="(cur, idx) in cursors" :key="idx">
      <span v-if="cur !== null" class="cursor-badge" :style="{ background: CURSOR_COLORS[idx] }">
        C{{ idx + 1 }}
      </span>
      <span v-if="cur !== null" class="cursor-time">{{ formatTime(cur, timeScale) }}</span>
      <span v-if="cur === null" class="cursor-empty">–</span>
    </div>
...
const props = defineProps({
  cursors:   { type: Array, required: true },    // [timestamp | null, ...]
