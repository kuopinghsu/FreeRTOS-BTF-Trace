...
export const AI_TOOL_CAPABILITY_VIEWER_MUTATION = 'viewer_mutation'
export const AI_TOOL_CAPABILITY_EXPORT = 'export'
export const AI_TOOL_CAPABILITY_STORAGE = 'storage'
export const AI_TOOL_CAPABILITY_HEURISTIC = 'heuristic'
export const AI_TOOL_CAPABILITY_READ_ONLY = 'read_only'

const AI_TOOL_CAPABILITY_BY_NAME = {
...
export function aiToolCapability(name) {
  const key = String(name || '').trim()
  return AI_TOOL_CAPABILITY_BY_NAME[key] || AI_TOOL_CAPABILITY_READ_ONLY
}

export function annotateToolCapabilities(tools = []) {
  return (tools || []).map((tool) => {
    if (!tool || typeof tool !== 'object') return tool
    const fn = tool.function && typeof tool.function === 'object' ? { ...tool.function } : null
    if (!fn) return { ...tool }
    const name = String(fn.name || '').trim()
    const cap = aiToolCapability(name)
    const desc = String(fn.description || '').replace(/\s+$/, '')
    const tag = `[capability: ${cap}]`
    if (!desc.includes(tag)) {
      fn.description = desc ? `${desc} ${tag}` : tag
    }
...
export function aiViewerTools() {
  ...
  return annotateToolCapabilities([
    {
      type: 'function',
      function: {
...
export function comparePerformanceTabs(baselineASummary, candidateBSummary, {
  labelA = 'A', labelB = 'B',
} = {}) {
  // First argument is Baseline A (Trace A), second is Candidate B (Trace B);
  // verdicts describe Candidate B versus Baseline A.
  const snapBase = snapshotFromSummary(baselineASummary || {}, { name: labelA })
  const snapCand = snapshotFromSummary(candidateBSummary || {}, { name: labelB })
  const ctx = comparePerformanceMetrics(snapBase, snapCand, { labelA, labelB })
  const ok = !!ctx.ok
  const message = String(ctx.message || (ok ? 'ok' : 'failed'))
  const data = { ...ctx }
  delete data.ok
