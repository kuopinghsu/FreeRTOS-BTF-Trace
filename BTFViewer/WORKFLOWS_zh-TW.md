...
BTFViewer 不會分析原始碼，也不會模擬 FreeRTOS 排程器；它只能量測追蹤檔內實際記錄的事件。若追蹤資料未包含必要事件，應清楚註明限制，不要推斷出看似精確的數值。
...
> **可見範圍注意事項：**核心遷移的 **Heatmap / Chord** 檢視器使用時間軸目前顯示的範圍，不會跟隨 **Limit to C1–Cn** 核取方塊。開啟檢視器前，請先縮放至游標範圍。
