...
**Settings → AI → Import…**
可以從 JSON 檔案載入預設集、Base URL、模型、API
金鑰與認證方式（範例見 `examples/ai/*.json`）。清單中尚未出現的預設集名稱——例如
`openrouter` 或 `deepseek`——會新增為獨立的預設集，而不會被併入
Custom。每個匯入的預設集都各自保留自己的 Base URL、模型、API
金鑰、認證方式與 TLS 設定，彼此獨立，也獨立於內建預設集。

匯入會立即更新 Settings
對話框，但在按下 **OK** 之前，不會寫入磁碟（`btf_viewer.rc`）或瀏覽器儲存空間。**Cancel**
會捨棄這次匯入。

再次匯入同一個預設集——經過大小寫與符號正規化後視為相同，例如
`OpenRouter` 與 `open-router`
會對應到同一筆——會更新該預設集已儲存的值，而不會產生重複項目。若檔案一次列出多個預設集，會全部加入，且不會移除先前匯入的預設集。
...
會將對話框各個分頁還原為內建預設值，包含版面配置、Statistics
呈現方式與 AI 設定。就 AI 而言，這表示：

-   透過 Import… 加入的每個預設集都會被移除；
-   預設集清單會還原為四個內建項目（Custom、Ollama、OpenAI、Gemini）；
-   目前的 AI 調查／對話內容會被清除；
-   已儲存的 API 金鑰與權杖會被清除，內建預設集與匯入的預設集都包含在內。

如同 Import，Reset to Defaults 只會變更對話框內容，直到按下
**OK** 才會生效。之後重新開啟 Settings 並按下
**Cancel**，先前的設定——預設集、認證資訊與對話內容——都不會受影響。
