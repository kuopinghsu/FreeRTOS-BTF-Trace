...
AI 是選用功能。即使不使用 AI，也可以透過統計資料、時間軸、Investigation
Notebook 與 Trace Compare 完成調查。
...
12. [完整工具參考](#complete-tool-reference)
13. [引擎限制](#engine-limits)
...
-   **Full
    Evidence**：提供更多證據；只有在前兩種模式缺少必要資訊時才需要使用。
...
AI
面板提供數個常用操作。可以把它們理解成不同類型的問題，而不是不同的分析系統。
...
| 5 | `find_critical_path` / `detect_priority_inversion` | 必要時檢查搶占、阻塞與優先權反轉路徑 |
...
5.  如果問題涉及較長的事件序列，再用 C1--Cn 圈出範圍，接著使用 **Explain
    region**。
...
先看回應時間分布與
p95、p99、Max。圈出一個較慢的實例，再檢查執行時間、派送等待、阻塞與附近的搶占事件。AI
可以協助整理這些因素，但引用的數值仍應能回到統計資料核對。
...
先使用 **Migration & Corridor
Inspector**。檢查遷移次數、停留時間、來回遷移、負載平衡與目前路徑。AI
可以把遷移路徑與附近的排程或同步證據整理在一起，但單純發生遷移不代表一定有效能問題。
...
先開啟可比較的基準追蹤資料與候選追蹤資料，優先使用 **Trace
Compare**，再請 AI 解釋。AI
...
圈住一個具代表性的離群事件，確認問題是否重複出現，以及是否與阻塞、搶占、遷移或特定工作負載階段同時發生。
...
5.  啟用 **Limit to C1--Cn**。
6.  請 AI 調查目前選取的分析結果。
7.  檢查延遲是否與執行、派送等待、阻塞、搶占、遷移或其他已量測因素有關。
...
如果 **Evidence & Validation** 已經提供 **Next
check**，可直接沿用目前調查內容執行下一項檢查。需要改變問題、質疑目前解釋或要求補充說明時，再使用一般追問。
...
一般使用者不需要記住工具名稱。AI
助理會依需要自行選擇。這一節的目的是說明每一類工具到底在做什麼。
...
-   `find_critical_path`：檢查事件附近的阻塞、搶占與互斥鎖關係。
-   `build_task_dependency_graph`：整理追蹤資料中可觀察到的等待、搶占、遷移與優先權繼承關係。
...
| `build_task_dependency_graph` | 整理工作附近的同步、搶占、遷移與優先權繼承關係 | 不是完整 ISR 或核心物件關係圖 |
...
AI
面板會顯示目前使用本機或雲端服務。把追蹤資料衍生資訊送到遠端服務前，先確認目前的隱私狀態。

雲端隱私處理可以清理部分註解內容，並依設定替換工作名稱。敏感設定可禁止傳送到雲端。

### 認證資訊

使用目前 BTFViewer 與服務提供者支援的認證方式。不要把 API
金鑰放進追蹤資料、報告、螢幕截圖或共享的調查檔案。
...
檢查 **Settings → AI**、端點、模型、認證資訊，並執行 **Test
connection**。
...
**Test
connection**。如果端點本身無法正常回應，應先修正端點，再檢查調查流程。
...
BTFViewer 使用一份共用的 **Investigation Case**（`btf-investigation-case`）保存目前調查狀態，讓不同 AI 操作可以延續同一項調查，不需要每次都只靠對話內容重新建立背景。
...
調查案例的用途是讓分析可以延續，**不是讓 AI 結論自動成為事實**。真正重要的仍是能回到 BTFViewer 核對的證據。
...
簡潔的工程紀錄可放在 Investigation Notebook；需要分享完整敘述時再匯出 AI
報告。不要把大量內部工具紀錄當成主要報告內容。
...
AI
文件也包含自動驗證所需的工程流程。這些命令與實際支援項目必須再與目前的
BTFViewer 命令列／桌面程式實作確認，不能只根據 `btf_viewer.html` 判定。
